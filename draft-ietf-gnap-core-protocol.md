...
`logo_uri` (string):
: Display image to represent the client software. OPTIONAL.
...
: An array of subject identifiers for the
            end user, as defined by {{I-D.ietf-secevent-subject-identifiers}}. OPTIONAL.
...
`error` (object):
: An error code indicating that something has gone wrong. REQUIRED for an error condition. If included, other fields MOST NOT be included. See {{response-error}}.
...
`redirect` (string):
: Redirect to an arbitrary URL. REQUIRED if the `redirect` interaction start mode is possible for this request. See {{response-interact-redirect}}.

`app` (string):
: Launch of an application URL. REQUIRED if the `app` interaction start mode is possible for this request. See {{response-interact-app}}.
...
`url` (string):
: The interaction URL that the client instance
